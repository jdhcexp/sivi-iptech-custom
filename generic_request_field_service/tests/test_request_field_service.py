...
class TestRequestFieldService(TestRequestFieldCase):
    ...
    def test_show_data_with_service_in_field_and_request(self):
        ...
        # Add service to type of requests
        self.field_type.write({
    ...
    def test_show_data_with_service_and_category_in_field_and_request(self):
        ...
        # Add service and categories to type of requests
        self.field_type.write({
            'service_ids': [(4, self.rent_nout_service.id)],
            'category_ids': [
                (4, self.request_category_demo_technical_config.id),
                (4, self.request_category_demo_resource.id),
            ]
        })
    ...
    def test_constrains_service_in_request_type(self):
        ...
        # Ensure ValidationError if remove service from type of requests
        with self.assertRaises(ValidationError):
            self.field_type.write({
                'service_ids': [(3, self.default_service.id)]
            })
