...
class TestRequestSLAService(RequestSLACase):
    ...
    def setUp(self):
        ...
        self.default_service.request_type_ids += self.sla_type
        self.default_service.category_ids += self.sla_type.category_ids

        self.rent_service = self.env.ref(
            'generic_service.generic_service_rent_notebook')
        self.rent_service.request_type_ids += self.sla_type
        self.rent_service.category_ids += self.sla_type.category_ids
    ...
    def test_onchange_service(self):
        ...
        request_service = self.env['generic.service'].create({
            ...
            'category_ids': [(4, self.request_category_support.id)],
            'request_type_ids': [(4, self.sla_type.id)]
        })
