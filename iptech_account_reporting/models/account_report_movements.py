...
class AccountReportMovements(models.Model):
	...
	CREATE OR REPLACE VIEW account_sql_report_movements AS (
            SELECT row_number() OVER() as id,
                                ...
                                'SERVICIO ENTREGADO ' || COALESCE(pt.name->>'en_US', 'Unknown') || ' ' || COALESCE(rp.street,'Unknown') || ' PERIODO FACTURADO ' || TO_CHAR( am.invoice_date, 'FMDD "de" TMMonth') || ' - ' || TO_CHAR( date_trunc('month', (aml.date + interval '1 month')) - interval '1 day','FMDD "de" TMMonth' ) ||' LUGAR DEL SERVICIO ' || COALESCE(rp.city,'Unknown') || ' DISPONIBILIDAD --' as a11,
